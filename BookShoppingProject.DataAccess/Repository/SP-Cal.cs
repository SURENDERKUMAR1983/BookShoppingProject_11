...
{
    public class SP_Cal : ISP_Cal
    {
        private readonly ApplicationDbContext _context;
        public static string connectionString = "";
        public SP_Cal(ApplicationDbContext context)
        {
            _context = context;
        ...
        public Tuple<IEnumerable<T1>, IEnumerable<T2>> List<T1, T2>(string procedureNmae, DynamicParameters param = null)
        {
            using(SqlConnection sqlCon=new SqlConnection(connectionString))
            {
                sqlCon.Open();
                var result = SqlMapper.QueryMultiple(sqlCon, procedureNmae, param, commandType: CommandType.StoredProcedure);
