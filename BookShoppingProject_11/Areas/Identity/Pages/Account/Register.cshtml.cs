...
namespace BookShoppingProject_11.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            Input = new InputModel()
            {
                ...
                RoleList = _roleManager.Roles.Where(r => r.Name != SD.Role_Admin).Select(x => x.Name).Select(rl => new SelectListItem()
                {
                    Text = rl,
                    Value = rl
                })
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if(!await _roleManager.RoleExistsAsync(SD.Role_Comapny))
                    {
                        await _roleManager.CreateAsync(new IdentityRole(SD.Role_Comapny)); 
                    }
                    ...
                    else
                    {
                        if(user.CompanyId>0)
                        {
                            await _userManager.AddToRoleAsync(user, SD.Role_Comapny);
                        }
                        else
                        {
